...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        //Amplitude Enable sending automatic session events
        Amplitude.instance().trackingSessionEvents = true
        // Initialize SDK
        Amplitude.instance().initializeApiKey("c7f3613610b36de38be70fd49ac3a4da")
        Amplitude.instance().setUserId(UserModel.main.userId)
        Amplitude.instance().logEvent("Open App")
        
        if !(AppUserDefaults.value(forKey: .firstOpenlogged).bool ?? false) {
            Amplitude.instance().logEvent("First Open")
            AppUserDefaults.save(value: true, forKey: .firstOpenlogged)
        }
