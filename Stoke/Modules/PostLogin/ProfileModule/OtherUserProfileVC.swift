    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 1{
            ...
            cell.showFollowers = { [weak self] flag in
                let type = flag ? "follower list": "following list"
                Amplitude.instance().logEvent("View Other User Follow Lists", withEventProperties: ["Type": type])
...
//                    self.showGuestUserPopup()
                    ...
                    if self.viewModel.model.isFollow{
                        ...
                        self.viewModel.followUser(!self.viewModel.model.isFollow, completion: {
                            self.viewModel.getProfile()
                            Amplitude.instance().logEvent("Follow", withEventProperties: ["Follow type": "Follow user"])
                        })
                    }
                }
            }
            ...
            cell.selectionMade = { [weak self] selection in
                Amplitude.instance().logEvent("View Other User \(selection.sectionName())")
...
extension OtherUserProfileVC {
    ...
    private func viewRecape(_ room:ChatRoom){
        ...
        vc.room = room
        Amplitude.instance().logEvent("View Recap", withEventProperties: ["Room Name": "\(room.name)"])
        AppRouter.pushViewController(self, vc)
    }
    ...
    private func moveToDetailScreen(_ room:ChatRoom){
        Amplitude.instance().logEvent("View Live Room Details")
