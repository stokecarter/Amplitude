...
//    }
    override func backButtonTapped() {
        Amplitude.instance().logEvent("Add User")
        ...
        ac.completionWithItemsHandler = { activity, success, items, error in
            ...
            if activity == UIActivity.ActivityType.postToFacebook {
                Amplitude.instance().logEvent("Send User Link", withEventProperties: ["Type": "postToFacebook"])
            }
            if activity == UIActivity.ActivityType.postToTwitter {
                Amplitude.instance().logEvent("Send User Link", withEventProperties: ["Type": "postToTwitter"])
            }
            if activity == UIActivity.ActivityType.copyToPasteboard {
                Amplitude.instance().logEvent("Send User Link", withEventProperties: ["Type": "copyToPasteboard"])
            }
            if activity == UIActivity.ActivityType.message {
                Amplitude.instance().logEvent("Send User Link", withEventProperties: ["Type": "message"])
            }
            if activity == UIActivity.ActivityType.mail {
                Amplitude.instance().logEvent("Send User Link", withEventProperties: ["Type": "mail"])
            }
        }
