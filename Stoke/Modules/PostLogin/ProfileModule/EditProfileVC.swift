...
class EditProfileVC: BaseVC {
    ...
    var bioUpdated: Bool = false
    var engagementBarUpdated: Bool = false
    var privateAccountUpdated: Bool = false
    var profilePictureUpdated: Bool = false
    ...
    override func rightBarButtonTapped(_ sender: UIButton) {
        if nameUpdated {
            Amplitude.instance().logEvent("Edit Name")
        }
        if bioUpdated {
            Amplitude.instance().logEvent("Edit Bio")
        }
        if engagementBarUpdated {
            Amplitude.instance().logEvent("Hide Engagement Bar")
        }
        if privateAccountUpdated {
            Amplitude.instance().logEvent("Private Account")
        }
        if profilePictureUpdated {
            Amplitude.instance().logEvent("Edit Profile Picture")
        }
    ...
    func cropViewControllerDidCrop(_ cropViewController: CropViewController, cropped: UIImage, transformation: Transformation) {
        ...
        AWSController.uploadImage(cropped, success: { [weak self](flag, url) in
            DispatchQueue.main.async {
                ...
                self?.profilePictureUpdated = true
            }
