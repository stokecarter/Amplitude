...
extension CategoriesEventListVC : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        switch indexPath.section {
        case 0:
            Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.upcomingEvents.liveEvents[indexPath.row].name)"])
            navigateToDetail(viewModel.upcomingEvents.liveEvents[indexPath.row])
        case 1:
            if indexPath.row != 0{
                Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.upcomingEvents.upcoming[indexPath.row - 1].name)"])
                navigateToDetail(viewModel.upcomingEvents.upcoming[indexPath.row - 1])
            }
        default:
            if indexPath.row != 0{
                Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.upcomingEvents.ended[indexPath.row - 1].name)"])
