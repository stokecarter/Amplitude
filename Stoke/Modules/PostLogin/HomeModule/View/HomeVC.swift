...
//    var hotTakeOptions: []
    ...
    override func viewDidLoad() {
        ...
        viewModel.update = {[weak self] in
            CommonFunctions.setUserProperties(user: (self?.viewModel.model)!)
        }
    }
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView)    {
        if self.scrollView === scrollView {
            ...
            case  UIDevice.width :
                ...
                Amplitude.instance().logEvent("View Hot Takes")
            default :
                break
            }
        }
            ...
            case 0:
                if indexPath.row == 0{
                    ...
                    cell.btnAction = {
                        ...
                        Amplitude.instance().logEvent("View All Events")
...
//                    cell.viewAllBtn.isHidden = true
            ...
            cell.upButtonTapped = {[weak self] in
                ...
                self?.viewModel.hitVoteAPI(postId: self?.viewModel.posts.data[indexPath.row].id ?? "", vote: 1) {//[weak self] in
                    Amplitude.instance().logEvent("Vote On Hot Take", withEventProperties: ["Type": "upvote"])
            ...
            cell.downButtonTapped = {[weak self] in
                ...
                let identify = AMPIdentify()
                    .set("No. Of Downvotes Given", value: NSNumber(value: (self?.viewModel.model.noOfDownVoteGiven ?? 0) + 1))
                Amplitude.instance().identify(identify)
                
                self?.viewModel.hitVoteAPI(postId: self?.viewModel.posts.data[indexPath.row].id ?? "", vote: -1) {//[weak self] in
                    Amplitude.instance().logEvent("Vote On Hot Take", withEventProperties: ["Type": "downvote"])
                    tableView.reloadRows(at: [indexPath], with: .automatic)
                }
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView === featuredTableView{
            ...
            case 0:
                if indexPath.row != 0{
                    Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.dataSource.liveEvents[indexPath.row - 1].name)"])
                    viewEventChatroomList(viewModel.dataSource.liveEvents[indexPath.row - 1])
                }
            case 1:
                if indexPath.row != 0{
                    Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.dataSource.upcoming[indexPath.row - 1].name)"])
                    viewEventChatroomList(viewModel.dataSource.upcoming[indexPath.row - 1])
                }
            default:
                if indexPath.row != 0{
                    Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.dataSource.ended[indexPath.row - 1].name)"])
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == calendarCV {
            DispatchQueue.main.async { [weak self] in
                ...
                Amplitude.instance().logEvent("View Different Day", withEventProperties: ["Day": "\(self.calender[indexPath.item].weekDay)"])
            ...
            switch HotTakeListType.allTypes()[indexPath.row] {
            case .top:
                Amplitude.instance().logEvent("View Top")
            case .new:
                Amplitude.instance().logEvent("View New")
            case .myTakes:
                Amplitude.instance().logEvent("View My Takes")
            case .allTime:
                Amplitude.instance().logEvent("View All Time")
            }
