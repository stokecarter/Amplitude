...
extension AllEventVC : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        switch indexPath.section {
        case 0:
            Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.allEvents.liveEvents[indexPath.row].name)"])
            viewEventChatroomList(viewModel.allEvents.liveEvents[indexPath.row])
        case 1:
            Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.allEvents.upcoming[indexPath.row - 1].name)"])
            viewEventChatroomList(viewModel.allEvents.upcoming[indexPath.row - 1])
        default:
            Amplitude.instance().logEvent("View Event", withEventProperties: ["Event Name": "\(viewModel.allEvents.ended[indexPath.row - 1].name)"])
