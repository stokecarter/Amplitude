...
class SearchVC: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        viewModel.reloadData = { [weak self] in
            ...
            if self.viewModel.searchStyle == .user{
                Amplitude.instance().logEvent("Search", withEventProperties: ["Search For": "User"])
                self.firstVC.updateEmptyScreen()
            }else if self.viewModel.searchStyle == .event{
                Amplitude.instance().logEvent("Search", withEventProperties: ["Search For": "Event"])
                self.secondVC.updateEmptyScreen()
            }else{
                Amplitude.instance().logEvent("Search", withEventProperties: ["Search For": "Tag"])
                self.thirdVC.updateEmptyScreen()
            }
